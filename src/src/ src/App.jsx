...
function generateSlotsForDay(date) {
  ...
  return slots.filter((s) => !(s.h === END_HOUR && s.m > 0));function normalizePhone(raw) {
...
async function insertBooking(booking) {
  ...
  if (error) console.error("insert booking failed", error);async function updateBookingRow(id, patch) {
  const dbPatch = {};
